...
// ===========================================
// AIZAWA ATTRACTOR WIDGET
// ===========================================
// A strange attractor discovered by Yoji Aizawa.
// Creates beautiful toroidal chaotic patterns that
// twist and fold through three-dimensional space.
// ===========================================
...
export default function AizawaAttractor() {
  ...
  // Compute Aizawa attractor points
  const computeAttractor = useCallback(
    (p: AttractorParams, numPoints: number = 80000) => {
      const { a, b, c, d, e, f } = p
      ...
      for (let i = 0; i < numPoints; i++) {
        ...
        points.push(new THREE.Vector3(x * 4, y * 4, z * 4))
      }
  ...
  useEffect(() => {
    ...
    // Color gradient along the line - magenta to cyan
    const colors: number[] = []
    for (let i = 0; i < points.length; i++) {
      const t = i / points.length
      // Magenta to cyan gradient
      colors.push(0.9 - t * 0.6, 0.2 + t * 0.7, 0.6 + t * 0.4)
    }
    ...
    const animate = () => {
      ...
      if (lineRef.current && cameraRef.current) {
        cameraRef.current.position.z = 20 / zoomRef.current
            ...
            <div className="p-3 bg-[#f5f5f5] rounded-lg space-y-3">
              ...
              <div>
                <div className="flex justify-between mb-1">
                  ...
                  <span className="text-sm font-mono text-black/50">{params.c.toFixed(2)}</span>
                </div>
                <input
                  type="range"
                  min="0.2"
              ...
              <div>
                <div className="flex justify-between mb-1">
                  ...
                  <span className="text-sm font-mono text-black/50">{params.d.toFixed(2)}</span>
                </div>
                <input
                  type="range"
                  min="2.0"
                  ...
                  step="0.05"
                  value={params.d}
                  onChange={(e) => setParams((p) => ({ ...p, d: parseFloat(e.target.value) }))}
                  className="w-full h-2 bg-[#e5e5e5] rounded-full appearance-none cursor-pointer
                    [&::-webkit-slider-thumb]:appearance-none
                    [&::-webkit-slider-thumb]:w-5
                    [&::-webkit-slider-thumb]:h-5
              ...
              <div>
                <div className="flex justify-between mb-1">
                  ...
                  <span className="text-sm font-mono text-black/50">{params.e.toFixed(2)}</span>
                </div>
                <input
                  type="range"
                  min="0.05"
                  ...
                  step="0.01"
                  value={params.e}
                  onChange={(e) => setParams((p) => ({ ...p, e: parseFloat(e.target.value) }))}
                  className="w-full h-2 bg-[#e5e5e5] rounded-full appearance-none cursor-pointer
                    [&::-webkit-slider-thumb]:appearance-none
                    [&::-webkit-slider-thumb]:w-5
                    [&::-webkit-slider-thumb]:h-5
              ...
              <div>
                <div className="flex justify-between mb-1">
                  ...
                  <span className="text-sm font-mono text-black/50">{params.f.toFixed(2)}</span>
                </div>
                <input
                  type="range"
                  min="0.0"
